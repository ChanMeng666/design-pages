...
//
// Adapted for Shaderview by Chan Meng for Design Pages
...
vec4 Eye(vec2 uv, float side, vec2 m, float smile) {
    ...
    d = length(uv-m*.45);                                      // offset pupil to look at mouse cursor
...
void main() {
    ...
    // Convert uPosition (pixel offset from center) to normalized coordinates
    vec2 m = uPosition / uResolution.xy;
    
    // If mouse hasn't moved (position is 0,0), make smiley look around
    if(length(uPosition) < 1.0) {
        float s = sin(t*.5);
        float c = cos(t*.38);
        m = vec2(s, c)*.4;
    }
